	...
	if not game:IsLoaded() then
		...
		GameLoadGui.Text = 'Đợi tí đang load :))';
		game.Loaded:Wait();
		GameLoadGui:Destroy();
		wait(10);
	end;
...
function TPB(CFgo)
	...
	local info = TweenInfo.new((game:GetService("Workspace").Boats[BoatSeaQ].VehicleSeat.CFrame.Position - CFgo.Position).Magnitude/300, Enum.EasingStyle.Linear)
	tween = tween_s:Create(game:GetService("Workspace").Boats[BoatSeaQ].VehicleSeat, info, {CFrame = CFgo})
	tween:Play()
	...
	function HopServer()
		if not _G.TP_Ser then
			...
			game.StarterGui:SetCore("SendNotification", {
				Title = "Dương modder ", 
				Text = "Đang tìm sever",
				Icon = "http://www.roblox.com/asset/?id=18715936839",
				Duration = 25
			})
			local PlaceID = game.PlaceId
			local AllIDs = {}
...
SOMEXHUBMODILE.Name = "SOMEXHUBMODILE"
        ...
        MODILEMAGE.Image = "http://www.roblox.com/asset/?id=18715936839"
...
local LogoHub = Instance.new("ImageLabel")
	...
	LogoHub.Image = "http://www.roblox.com/asset/?id="..tostring(18715936839)
...
local LogoToggle = Instance.new("ImageLabel")
	...
	LogoToggle.Image = "http://www.roblox.com/asset/?id="..tostring(18715936839)
		...
		function main:Dropdown(text,option,callback)
			...
			DropImage.Image = "rbxassetid://18715936839"
...
local Library = Update:Window("Dương","",Enum.KeyCode.RightControl);

Main = Library:Tab("Main",6026568198)
Sta = Library:Tab("Nâng điểm",7040410130)
Miscf = Library:Tab("Farm",9606626859)
Sho = Library:Tab("Trái ác quỷ",7044233235)
Shop = Library:Tab("Cửa hàng",9606625251)
Raid = Library:Tab("Raid",9606629300)
TPKub = Library:Tab("Dịch chuyển",9606628205)
Py = Library:Tab("PvP",9606626034)



Main:Toggle("Tự động farm level",_G.Setting_table.AutoFarm,function(vu)
	Auto_Farm = vu
	Auto_Fast_Farm = vu
...
Main:Toggle("Tự động farm điểm kĩ năng lên 600",_G.Setting_table.MasMelee,function(vu)
	MasMelee = vu
...
Main:Seperator("Tự động sang biển mới")

Main:Toggle("Tự động lên thế giới mới",_G.Setting_table.MasMelee,function(vu)
    Auto_New_A = vu
...
Main:Toggle("Tự động sang sea 3",false,function(vu)
Auto_Three_World = vu
end)
								...
								if game.Workspace.Enemies:FindFirstChild(namemon) then
									...
									repeat task.wait()
										for i,v in pairs(game.Workspace._WorldOrigin.EnemySpawns:GetChildren()) do
											if string.find(v.Name,NotFindGray) then
...
Main:Dropdown("Chọn trang bị", Waspon,function(vu)
...
Main:Dropdown("Bật chế độ nhanh",fast,function(vu)
...
Main:Dropdown("Lựa chọn khoảng cách độ trễ",SFast,function(vu)
	_G.Setting_table.FastModeD = vu
	Update_Setting(getgenv()['MyName'])
end)

Main:Toggle("Nhiệm vụ kép ",_G.Setting_table.DbQ,function(vu)
	DbQ = vu
...
Main:Toggle("Bỏ qua hoạt ảnh dịch chuyển(bypass)",_G.Setting_table.KobenHeegeen,function(vu)
	Bypass = vu
	_G.Setting_table.Bypass = vu
	Update_Setting(getgenv()['MyName'])
end)

Main:Toggle("Tắt hiển thị sát thương ",_G.Setting_table.KobenHeegeen,function(vu)
	KobenHeegeen = vu
...
Main:Toggle(" Loại bỏ thông báo ",_G.Setting_table.Refec,function(vu)
	Refec = vu
...
Main:Toggle(" Loại bỏ hiệu ứng mob ",_G.Setting_table.Refec,function(vu)
	Remove_Effect = vu
...
Main:Toggle(" Loại bỏ âm thanh ",_G.Setting_table.Refec,function(vu)
	RemoveHit = vu
...
Main:Toggle(" Xe buýt tự động ",_G.Setting_table.Auto_Buso,function(vu)
	Auto_Buso = vu
...
Main:Toggle(" Tự động bật haki ",_G.Setting_table.AutoKen,function(vu)
...
Main:Toggle(" Chuyển sang màn đen(giảm nóng máy) ",_G.Setting_table.StartBlackScreen,function(vu)
...
Main:Toggle("Mob vô hình",_G.Setting_table.inv,function(value)
...
Code = {
    ...
    Main:Toggle(" Tự động nhập code ",_G.Setting_table.Auto_Redeem_All_Code,function(vu)
    ...
    Main:Slider("Số lượng code muốn nhập",0,2450,1,function(value)
...
Main:Seperator("Farm quái")

Main:Toggle("Tự động farm quái đã chọn",_G.Setting_table.Auto_Farm_Monster,function(vu)
...
Main:Toggle("Tự động lụm rương",_G.Setting_table.Grab_Chest,function(vu)
	Grab_Chest = vu
...
Main:Toggle("Tự động lụm rương trên đảo bí ẩn",_G.Setting_table.AutoChestMirage,function(vu)
	...
	spawn(function()
		pcall(function()
			while wait() do
				Bone:Set("    // Xương hiện có : "..tostring(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones","Check").." \\\\"))
			end
		end)
	end)

	Main:Toggle("Farm xương(sea3)",_G.Setting_table.Farm_Bone,function(vu)
		Auto_Farm_Bone = vu
	...
	Main:Toggle("Random Xương(sea3)",_G.Setting_table.Farm_Random_S,function(vu)
		Farm_Random_S = vu
 ...
 Elite_Hunter_Status = Main:Label("")
	...
	Main:Toggle("Tự động đánh nữ hoàng đảo bánh V2",_G.Setting_table.Prince2,function(vu)
		Prince2 = vu
		_G.Setting_table.Prince2 = vu
		Update_Setting(getgenv()['MyName'])
	end)

	Main:Toggle("Hop Đánh nữ hoàng bánh V2",_G.Setting_table.Hop,function(vu)
		Hop = vu
										...
										if not _G.TP_Ser then
											...
											function TPReturner()
												...
												for i,v in pairs(Site.data) do
													...
													game.StarterGui:SetCore("SendNotification", {
														Title = "Dương Modder ", 
														Text = "Số lượng user : " ..tonumber(v.playing),
														Icon = "http://www.roblox.com/asset/?id=18715936839",
														Duration = 1.5
													})
													if tonumber(v.maxPlayers) > tonumber(v.playing) then
	...
	Main:Seperator("Khu vực cài đặt Farm")

	Main:Toggle("Tự động dò trái ác quỷ",_G.Setting_table.Farm_Fruit,function(vu)
		Auto_Farm_Fruit = vu
    ...
    MinHealth = 20
	Main:Slider("Khoảng cách với mob(quái)",1,100,20,function(vu)
		MinHealth = vu
		_G.Setting_table.MinHealth = vu
		Update_Setting(getgenv()['MyName'])
	end)

	Main:Slider("Khoảng cách Farm ",1,5000,1000,function(vu)
...
Miscf:Toggle("Dịch chuyển đến bánh răng",_G.Setting_table.TeleporttoGear,function(vu)
...
Miscf:Toggle("Tự động tìm trăng tròn",_G.Setting_table.AutoFindFullmoon,function(vu)
...
Miscf:Toggle("Quái xung quanh",_G.Setting_table.Mob_Aura,function(vu)
	AutoMobAura = vu 
	_G.Setting_table.Mob_Aura = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Slider("Khoảng cách farm",1,5000,1000,function(vu)
	DistanceMobAura = vu
...
Miscf:Toggle("Tự động đánh Boss",_G.Setting_table.AutoFarmBoss,function(vu)
	_G.Setting_table.AutoFarmBoss = vu
	Update_Setting(getgenv()['MyName'])
end)


Miscf:Toggle("Tự động farm tất cả Boss",_G.Setting_table.AutoFarmAllBoss,function(vu)
...
Miscf:Toggle("Tự động làm nhiệm vụ Boss",_G.Setting_table.AutoQuestBoss,function(vu)
...
Miscf:Seperator("// Kiểu chiến đấu \\\\\ ")

Miscf:Toggle("Tự động lấy Superhuman",_G.Setting_table.Superhuman,function(vu)
	_G.Setting_table.Superhuman = vu
	Update_Setting(getgenv()['MyName'])
end)
Miscf:Toggle("Tự động lấy Death Step v2",_G.Setting_table.Death_Step,function(vu)
	_G.Setting_table.Death_Step = vu
	
	Update_Setting(getgenv()['MyName'])
end)
Miscf:Toggle("Tự động lấy Sharkman Karate v2",_G.Setting_table.Sharkman_Karate,function(vu)
	_G.Setting_table.Sharkman_Karate = vu
	
	Update_Setting(getgenv()['MyName'])
end)
Miscf:Toggle("Tự động lấy Electric Claw v2",_G.Setting_table.Electric_Claw,function(vu)
	_G.Setting_table.Electric_Claw = vu
	
	Update_Setting(getgenv()['MyName'])
end)
Miscf:Toggle("Tự động lấy Dragon Talon",_G.Setting_table.Dragon_Talon,function(vu)
	_G.Setting_table.Dragon_Talon = vu
	
	Update_Setting(getgenv()['MyName'])
end)
Miscf:Toggle("Tự động lấy God Human",_G.Setting_table.Auto_God_Human,function(vu)
...
Miscf:Toggle("Tự động farm nguyên liệu",_G.Setting_table.AutoFarm_Meterial,function(vu)
...
Miscf:Seperator("// Săn boss bí ẩn \\\\\ ")

Miscf:Toggle("Tự động đánh boss bí ẩn",_G.Setting_table.Auto_Elite_Hunter,function(vu)
...
Miscf:Seperator("// Gậy lôi \\\\\ ")

Miscf:Toggle("Tự động lấy gậy lôi V1",_G.Setting_table.Pole_Farm,function(vu)
	Pole_Farm = vu
...
Miscf:Toggle("Tự động lấy gậy lôi V1(Hop)",_G.Setting_table.Pole_V1_Hop,function(vu)
	_G.Setting_table.Pole_V1_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Tự động lấy cung Serpent \\\\\ ")

Miscf:Toggle("Tự động lấy cung Serpent",_G.Setting_table.Auto_Serpent_Bow,function(vu)
	Auto_Serpent_Bow = vu
...
Miscf:Toggle("Tự động lấy cung Serpent(Hop)",_G.Setting_table.Auto_Serpent_Bow_Hop,function(vu)
	_G.Setting_table.Auto_Serpent_Bow_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Tự động lấy Dragon Trident \\\\\ ")
...
Miscf:Toggle("Tự động lấy Dragon Trident(Hop)",_G.Setting_table.Auto_Dragon_Trident_Hop,function(vu)
	_G.Setting_table.Auto_Dragon_Trident_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Kính Swan  \\\\\ ")

Miscf:Toggle("Tự động lấy kính Swan",_G.Setting_table.Auto_Swan_Glass,function(vu)
	Auto_Swan_Glass = vu
...
Miscf:Toggle("Tự động lấy kính Swan(Hop)",_G.Setting_table.Auto_Swan_Glass_Hop,function(vu)
	_G.Setting_table.Auto_Swan_Glass_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Kiếm Buddy \\\\\ ")

Miscf:Toggle("Tự động lấy kiếm Buddy",_G.Setting_table.Auto_Buddy_Sword,function(vu)
	Auto_Buddy_Sword = vu
...
Miscf:Toggle("Tự động lấy kiếm Buddy(Hop)",_G.Setting_table.Auto_Buddy_Sword_Hop,function(vu)
	_G.Setting_table.Auto_Buddy_Sword_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Yoru mini-Mũ Rip-Indra \\\\\ ")

Miscf:Toggle("Tự động lấy Mũ Rip-Indra",_G.Setting_table.Auto_Valkyrie_Helmet,function(vu)
...
Miscf:Toggle("Tự động lấy Mũ Rip-Indra(Hop)",_G.Setting_table.Auto_Valkyrie_Helmet_Hop,function(vu)
	_G.Setting_table.Auto_Valkyrie_Helmet_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)
if Three_World then
Miscf:Seperator("// Khu vực biển động \\\\\ ")
Miscf:Label("Cài đặt")
...
Miscf:Dropdown("Chọn thuyền bạn muốn đi khám phá biển động:",AllBoat,function(value)
BoatSeaQ = value
end)


_G.SelectLocalTeleportSea = "Sea6[???]"
Miscf:Dropdown("Chọn Sea",{"Sea1[Low]","Sea2[Medium]","Sea3[High]","Sea4[Extreme]","Sea5[Crazy]","Sea6[???]"},function(value)
_G.SelectLocalTeleportSea = value
end)

Miscf:Toggle("Auto Sailing Boat",_G.Setting_table.BuyBoatDmd,function(vu)
    BuyBoatDmd = vu
	_G.Setting_table.BuyBoatDmd = vu
	Update_Setting(getgenv()['MyName'])
end)
            ...
            pcall(function()
                if BuyBoatDmd then
                    if not game:GetService("Workspace").Enemies:FindFirstChild("Shark") or not game:GetService("Workspace").Enemies:FindFirstChild("Terrorshark") or not game:GetService("Workspace").Enemies:FindFirstChild("Piranha") or not game:GetService("Workspace").Enemies:FindFirstChild("Fish Crew Member") then
                        if not game:GetService("Workspace").Boats:FindFirstChild(BoatSeaQ) then
                        ...
                        elseif game:GetService("Workspace").Boats:FindFirstChild(BoatSeaQ) then
                            if game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Sit == false then
                                TPP(game:GetService("Workspace").Boats[BoatSeaQ].VehicleSeat.CFrame * CFrame.new(0,1,0))
                            else
                                for i,v in pairs(game:GetService("Workspace").Boats:GetChildren()) do
                                    if v.Name == BoatSeaQ then
                                        repeat wait()
                                        ...
                                        until game:GetService("Workspace").Enemies:FindFirstChild("Shark") or game:GetService("Workspace").Enemies:FindFirstChild("Terrorshark") or game:GetService("Workspace").Enemies:FindFirstChild("Piranha") or game:GetService("Workspace").Enemies:FindFirstChild("Fish Crew Member") or BuyBoatDmd == false
                                    end
                                end
                            end
                        end
...
_G.BoatSpeed = "350"
Miscf:Dropdown("Cài đặt tốc độ thuyền(mặc định 350)",SpeedTable,function(value)
_G.BoatSpeed = value
end)

    
    Miscf:Toggle("Tăng tốc độ thuyền",_G.Setting_table.IncreaseBoatSpeed,function(vu)
        _G.Setting_table.IncreaseBoatSpeed = vu
	    Update_Setting(getgenv()['MyName'])
    local vehicleSeats = {}
                ...
                if _G.Setting_table.IncreaseBoatSpeed then
...
Miscf:Toggle("Tự động làm tất cả nhiệm vụ biển [BETA]", _G.Setting_table.AutoRougthFully,function(vu)
         AutoRougthFully = vu
                    ...
                    for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v.Name == "PirateBrigade" or v.Name == "PirateGrandBrigade" or v.Name == "FishBoat" then
                            if v.Parent and v:FindFirstChild("Health") and v.Health.Value > 0 then
                                repeat task.wait()
                                    TP(v.VehicleSeat.CFrame * CFrame.new(0, 30, -20))
                                    ...
                                    until not v.Parent or not v:FindFirstChild("Health") or not v.Health.Value > 0 or AutoRougthFully == false
                                    Skillaimbot = false
                                end
                            end
                        end
                            ...
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                                if v.Name == "Shark" or v.Name == "Terrorshark" or v.Name == "Piranha" or v.Name == "Fish Crew Member" then
                                    repeat task.wait()
                                        ...
                                        FastAttack = true
                                    until v.Humanoid.Health <= 0 or not v.Parent or AutoRougthFully == false
                                    FastAttack = false
                            end
                        end
                    end
                elseif game:GetService("Workspace").SeaBeasts:FindFirstChild("SeaBeast1") then  
                    for i,v in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do
                        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Health") and v.Health.Value > 0 then
                            ...
                            until not v:FindFirstChild("HumanoidRootPart") or not v:FindFirstChild("Health") or not v.Health.Value > 0 or AutoRougthFully == false
                            Skillaimbot = false
                        end
                    end
                end
    ...
    Miscf:Toggle("Tự động Farm Terrorshark", _G.Setting_table.AutoTerrorshark,function(vu)
         AutoTerrorshark = vu
         _G.Setting_table.AutoTerrorshark = vu
         Update_Setting(getgenv()['MyName'])
    end)
    
    Miscf:Toggle("Tự động farm cá mập", _G.Setting_table.FarmShark,function(vu)
         FarmShark = vu
    ...
    Miscf:Toggle("Tự động farm cá Piranha", _G.Setting_table.FarmPiranha,function(vu)
         FarmPiranha = vu
    ...
    pcall(function()
        while wait() do
            if game:GetService("Workspace").Map:FindFirstChild('KitsuneIsland') then
                YESS:Set('🟢: Đảo kitsune đang spawm')
            else
                YESS:Set('🔴: Ddảo kitsune không spawm')
            end
        end
    end)
end)
    ...
    Miscf:Toggle("Dịch chuyển tới đảo Kitsune",_G.AutoFKitsune,function(value)
    AutoKisune = value
    _G.Setting_table.AutoKisune = value
    Update_Setting(getgenv()['MyName'])
    end)
    
    Miscf:Toggle("Tự động farm linh hồn(kitsune)",_G.AutoAzuerEmber,function(value)
    AutoEmber = value
    _G.Setting_table.AutoEmber = value
    Update_Setting(getgenv()['MyName'])
    end)
...
Miscf:Seperator("// Kiếm Saber \\\\\ ")

Miscf:Toggle("Tự động lấy Saber",_G.Setting_table.Saber_Farm,function(vu)
	Saber_Farm = vu
...
Miscf:Seperator("// Đánh hải tặc \\\\\ ")

Miscf:Toggle("Tự động đánh hải tặc trên đảo lâu đài",_G.Setting_table.Auto_Quest_Tushita_2,function(vu)
...
Miscf:Seperator("// Kiếm tushita \\\\\ ")

Miscf:Toggle("Tự động lấy kiếm Tushita",_G.Setting_table.Auto_Tushita,function(vu)
	Auto_Tushita = vu
...
Miscf:Seperator("// Kiếm Twin Hooks \\\\\ ")

Miscf:Toggle("Tự động lấy kiếm Twin Hooks",_G.Setting_table.Auto_Twin_Hooks,function(vu)
	Auto_Twin_Hooks = vu
...
Miscf:Toggle("Tự động lấy kiếm Twin Hooks(Hop)",_G.Setting_table.Auto_Twin_Hooks_Hop,function(vu)
	_G.Setting_table.Auto_Twin_Hooks_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Kiếm Canvander \\\\\ ")


Miscf:Toggle("Tự động lấy kiếm Canvander",_G.Setting_table.Auto_Canvander,function(vu)
	Auto_Canvander = vu
...
Miscf:Toggle("Tự động lấy kiếm Canvander(Hop)",_G.Setting_table.Auto_Canvander_Hop,function(vu)
	_G.Setting_table.Auto_Canvander_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

    
Miscf:Seperator("// Lưỡi hái Hallow Scythe \\\\\ ")

Miscf:Toggle("Tự động lấy lưỡi hái Hallow Scythe",_G.Setting_table.Auto_Hallow_Scryte,function(vu)
...
Miscf:Toggle("Tự động lấy lưỡi hái Hallow Scythe(Hop)",_G.Setting_table.Hallow_Scryte_Hop,function(vu)
	_G.Setting_table.Hallow_Scryte_Hop = vu
	Update_Setting(getgenv()['MyName'])
end)

Miscf:Seperator("// Yoru-Mini \\\\\ ")

Miscf:Toggle("Tự động lấy Yoru-Mini",_G.Setting_table.Auto_Dark_Dagger,function(vu)
	Auto_Dark_Dagger = vu
...
Miscf:Toggle("Tự động lấy Yoru-Mini(Hop)",_G.Setting_table.Auto_Dark_Dagger_Hop,function(vu)
...
LegendSwords = Miscf:Label("")
    ...
    Miscf:Toggle("Tự động mua kiếm huyền thoại",_G.Setting_table.AutoBuyLegendarySword,function(vu)
...
Py:Seperator("Bounty")
	...
	Py:Toggle("Auto Aimbot Gun(Beta)",false,function(vu)
		Aimbot = vu
		if vu then
			for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
				if v:IsA("Tool") then
	...
	Py:Toggle("Auto Aimbot skill(Beta)",false,function(vu)
		Skillaimbot = vu
		if Skillaimbot then
...
Main:Label("This script made by Dương Coder")
...
task.spawn(function()
	while task.wait() do
		pcall(function()
			if _G.Setting_table.FastModeD == "Trung Bình" then
				Faa:Set("Tấn công | 1 Wait | 1")
			elseif _G.Setting_table.FastModeD == "Nhanh" then
				Faa:Set("Tấn công | 2.5 Wait | 1")
			elseif _G.Setting_table.FastModeD == "Siêu nhanh" then
				Faa:Set("Tấn công | INF Wait | 0")
			end
		end)
	end
end)
...
task.spawn(function()
	while task.wait() do
		pcall(function()
			if DbFast then
				db:Set("Tấn công Nhanh X2 | Bật ✅")
			else
				db:Set("Tấn công Nhanh X2 | Tắt ❌")
			end
		end)
	end
end) 
	...
	while task.wait() do
		pcall(function()
			if game.Workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island") then
				mr:Set("Đảo bí ẩn : Đang có ✅")
			else
				mr:Set("Đảo bí ẩn : Chưa có ❌")
			end
		end)
	end
end) 